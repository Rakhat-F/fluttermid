...
void allOperations() {
  if (_parameters[_parameters.length - 1] == "+") {
    ...
    double percent = percentage(num1List.join(), num2List.join());
    num1List = ["$percent"];
    num2List = [];
    result = percent;
  }
}
percentage(String num1, String num2){
  if(double.parse(num1) > double.parse(num2)){
    double perResult = double.parse(num2) / double.parse(num1);
    perResult *= 100;
    return perResult;
  }
  else{
    double perResult = double.parse(num1) / double.parse(num2);
    perResult *= 100;
    return perResult;
  }
}
